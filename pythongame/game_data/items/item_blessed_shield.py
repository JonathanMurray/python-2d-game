...
class ItemEffect(StatModifyingItemEffect):
    ...
    def __init__(self, proc_chance: float, healing_amount: int, item_type: ItemType, stat_modifiers):
...
def register_blessed_shield_item():
    ...
    for i in range(3):
        ...
        description = effect.get_description() + [
            "{:.0f}".format(proc_chance * 100) + "% on hit: gain " + str(healing_amount) + " health"]
