...
from pythongame.core.npc_behaviors import register_npc_behavior, AbstractNpcMind, register_npc_dialog_data, DialogData, \
    DialogOptionData, AbstractNpcAction
...
class EndAction(AbstractNpcAction):
    def act(self, game_state: GameState):
        game_state.player_state.gain_upgrade(HeroUpgrade.HAS_WON_GAME)


def register_challenge_finisher_npc():
    size = (30, 30)  # Must not align perfectly with grid cell size (pathfinding issues)
    sprite = Sprite.NEUTRAL_NPC_CHALLENGE_FINISHER
    portrait_icon_sprite = PortraitIconSprite.CHALLENGE_FINISHER
    npc_type = NpcType.NEUTRAL_CHALLENGE_FINISHER
    movement_speed = 0.03
    register_npc_data(npc_type, NpcData.neutral(sprite, size, movement_speed))
    register_npc_behavior(npc_type, NpcMind)

    dialog_options = [DialogOptionData("> Finish", "end game", EndAction())]
    dialog_text_body = "You win!"
